...
/**
 *  @brief      Get all packets from the FIFO. (Lujano implementation)
 *  This function should be used if the packet is to be parsed elsewhere.
 *  @param[in]  length  Length of one FIFO packet.
 *  @param[in]  data    FIFO packets.
 *  @param[in]  more    Number of packets readed.
 */
int mpu_read_fifo_all_packets(unsigned short length, unsigned char *data,
    unsigned char *packets_readed)
{
    unsigned char tmp[2];
    unsigned short fifo_count;
    ...
    int indexBuffer = 0;
    int i, j;
    for( i= 0; i < fifo_count / length; i++) // paquetes a ser leidos
    {
        unsigned char packet[length]; // espacio de memoria para copiar paquete
        if (i2c_read(st.hw->addr, st.reg->fifo_r_w, length, packet)) // intentar leer paquete
        return -1;
        
        for( j = 0; j < length; j++) // Copiar paquete al buffer de datos
        {
            data[indexBuffer+j] = packet[j];
        }
        indexBuffer = indexBuffer+length;

    }
    
    packets_readed[0] = fifo_count / length;
    return 0;
}
