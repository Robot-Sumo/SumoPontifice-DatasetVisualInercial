...
int  MPUManager::__LoadDataFromDevice( __CaptureData_t *p_pData )
{
	...
	if( 0 == __mpu_get_fifo_bytes_count(&ucLen) && ucLen >= m_usPackageLength )
	{
		...
		if( 0 == dmp_read_fifo( sDataGyro, sDataAccel, lQuaternion, &p_pData->m_ulTimestamp, &sMask, &ucMore ) )
		{
			...
			*/
			...
			return int(ucMore); // retornar numero de paquetes restantes
		}
	}
...
int  MPUManager::__LoadAllDataFromDevice( __CaptureData_Buffer_t *p_pData ) // Lujano implementation
{
	// Acc and gyro buffers
	static short sDataGyro[3*100] = {0};
	static short sDataAccel[3*100] = {0};
	static unsigned char packets_readed = 0;
	...
	if( 0 == dmp_read_all_fifo_gyro_acc( sDataGyro, sDataAccel,  &packets_readed) )
	{
		...
		for(int i=0; i < packets_readed; i++)
		{
	
			
			if( m_usDefaultFeatures&DMP_FEATURE_SEND_CAL_GYRO )
			{
				p_pData->dataGyro[indexGyro] = (float)sDataGyro[indexGyro]/m_fCurGyroSensitivity; // g
				p_pData->dataGyro[indexGyro+1] = (float)sDataGyro[indexGyro+1]/m_fCurGyroSensitivity;
				p_pData->dataGyro[indexGyro+2] = (float)sDataGyro[indexGyro+2]/m_fCurGyroSensitivity;
				indexGyro =indexGyro+3;
			}

			if( m_usDefaultFeatures&DMP_FEATURE_SEND_RAW_ACCEL )
			{ 
				p_pData->dataAccel[indexAcc] = (float)sDataAccel[indexAcc]/m_usCurAccelSensitivity; // DPS
				p_pData->dataAccel[indexAcc+1] = (float)sDataAccel[indexAcc+1]/m_usCurAccelSensitivity;
				p_pData->dataAccel[indexAcc+2] = (float)sDataAccel[indexAcc+2]/m_usCurAccelSensitivity;
				indexAcc =indexAcc+3;
			}

					//timestamp
			p_pData->timestamp[i] = timestampCurrentPacket;
			timestampCurrentPacket = timestampCurrentPacket+1000000000/m_usSampleRate; //ns
...
{
	__CaptureData_Buffer_t _Data;

	memset( &_Data, 0, sizeof(__CaptureData_Buffer_t) );
	timestampInit = timestamp.getNanoSecs();
	timestampCurrentPacket = timestampInit;
	std::cout << "time init " << timestampCurrentPacket <<std::endl;
...
bool MPUManager::SetUpdateDataType( MPUManager::__UpdateDataType p_iType, void (*p_pfunc)(const __CaptureData_Buffer_t *) )
{
	if( m_bRunning )
	{
		return false;
