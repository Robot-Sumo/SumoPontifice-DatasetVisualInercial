...
/**
 *  @brief      Get all packets from the FIFO.
 *  If @e sensors does not contain a particular sensor, disregard the data
 *  returned to that pointer.
 *  \n @e sensors can contain a combination of the following flags:
 *  \n INV_X_GYRO, INV_Y_GYRO, INV_Z_GYRO
...
{
    unsigned char fifo_data[100*MAX_PACKET_LENGTH];
    unsigned char ii = 0;
    ...
    /* Get a packet. */
    if (mpu_read_fifo_all_packets(dmp.packet_length, fifo_data, packets_readed))
        return -1;

    for (i = 0; i <*packets_readed ; i++ )
    {
        if (dmp.feature_mask & DMP_FEATURE_SEND_RAW_ACCEL) {
            accel[indexAcc] = ((short)fifo_data[ii+0] << 8) | fifo_data[ii+1];
            accel[indexAcc+1] = ((short)fifo_data[ii+2] << 8) | fifo_data[ii+3];
            accel[indexAcc+2] = ((short)fifo_data[ii+4] << 8) | fifo_data[ii+5];
            ii += 6;
        }

        if (dmp.feature_mask & DMP_FEATURE_SEND_ANY_GYRO) {
            gyro[indexGyro] = ((short)fifo_data[ii+0] << 8) | fifo_data[ii+1];
            gyro[indexGyro+1] = ((short)fifo_data[ii+2] << 8) | fifo_data[ii+3];
            gyro[indexGyro+2] = ((short)fifo_data[ii+4] << 8) | fifo_data[ii+5];
            ii += 6;
        }
