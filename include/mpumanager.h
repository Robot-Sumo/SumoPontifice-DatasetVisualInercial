...
struct __CaptureData_Buffer_t
{
	float dataGyro[3*100];		// x y z
	float dataAccel[3*100];		// x y z
	float temperature;
...
private:
	...
	int __LoadDataFromDevice( __CaptureData_t *p_pData );
	int __LoadAllDataFromDevice( __CaptureData_Buffer_t *p_pData );
	...
	int numPackages2read;
	Timestamp timestamp;
	int64_t timestampInit; // timestampinicial
	int64_t timestampCurrentPacket; // timestamp del paquete actual
	double time1, time2;
...
public:
	...
	bool SetUpdateDataType( __UpdateDataType p_iType, void (*p_pfunc)(const __CaptureData_Buffer_t *) );
